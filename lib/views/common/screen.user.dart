...
class UserScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
	...
	// if [true] then the user will be treated as a pastor ignoring all other
	// keys in the map that does not belong to a pastor role
	final bool isPastor = params['is_pastor'] ?? false;

    // if [params] does not have a key named 'user' then the while object of [params]
    // is going to be threated as the user object
    final Map<String, dynamic> user =
        params.containsKey("user") ? params['user'] : params;
    ...
    return Scaffold(
      ...
      body: FutureBuilder<List<Map<String, dynamic>>>(
          future: API().queryUserByID(
                  [isPastor? user['idpastor']:user['id']], user['tipo'] ?? 'P'),
          builder: (context, snapshot) {
            ...
            if (snapshot.data!.isEmpty) {
              return const Center(
                  child: Text("No encontrado. Intente más tarde"));
            }
            ...
            if (user.containsKey("id")) {
              switch ((user['tipo'] as String).trim().toUpperCase()) {
                case "E":
                  String photoID = (userData['foto_esposa'] ?? "sin_foto.jpg")
                      .split('/')
                      .last;
                  photoUrl =
                      "https://oficial.cedeieanjesus.org/uploads/foto_esposa_pastor/$photoID";
                  break;
                case "P":
                  String photoID = (userData['foto_pastor'] ?? "sin_foto.jpg")
                      .split('/')
                      .last;
                  photoUrl =
              ...
              String photoID =
                  (userData['foto_pastor'] ?? "sin_foto.jpg").split('/').last;
              photoUrl =
                  "https://oficial.cedeieanjesus.org/uploads/foto_pastor/$photoID";
            }
            ...
            return ListView(
              ...
              children: [
                ...
                ListTile(
                  ...
                  subtitle:
                      Text("${userData["nacionalidad"] ?? "No disponible"}"),
                ),
                if (userData.containsKey("foto_hijo"))
                  ...loadPastorHijoData(context, userData),
                if (userData.containsKey("foto_esposa"))
                  ...loadPastorEsposaData(context, userData),
                if (userData.containsKey("foto_pastor"))
                  ...loadPastorData(context, size, userData),
              ],
            );
          }),
    );
  ...
  List<Widget> loadPastorData(BuildContext context, Size size, Map<String, dynamic> user) {
    ...
    return <Widget>[
      ...
      if (telefono.trim().isNotEmpty)
        ListTile(
          ...
          subtitle:
              Text(telefono.trim().isNotEmpty ? telefono : "No disponible"),
          trailing: SizedBox(
      ...
      ListTile(
		...
		onTap: (){
			Navigator.of(context).pushNamed(
				RouteNames.congregation.toString(),
				arguments: user["idcongregacion"]
			);
		}
      ),
      ListTile(
  ...
  List<Widget> loadPastorEsposaData(
    ...
    return <Widget>[
      ListTile(
        ...
        onTap: () {
          Navigator.of(context).pushNamed(RouteNames.user.toString(),
              arguments: {
                'id': user['id_pastor'],
  ...
  List<Widget> loadPastorHijoData(
      BuildContext context, Map<String, dynamic> user) {
    final size = MediaQuery.of(context).size;
    String telefono = user["telefono"] ?? "";
    ...
    return <Widget>[
      ...
      ListTile(
        ...
        subtitle: Text(
            "${(user["correo"] as String).trim().isNotEmpty ? user["correo"] : "No disponible"}"),
      ),
      ListTile(
        leading: const Icon(Icons.baby_changing_station),
        title: const Text("Fecha de nacimiento"),
